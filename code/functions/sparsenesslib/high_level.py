...
def preprocess_average(bdd,weight,metric, model_name, computer, freqmod,k = 1,computation = 'featureMap'):
    """!
        Sets up the components for the execution of Average.   
        adapts the input and output paths.
        preprocesses some databases (CFD_ALL, fairface)
    ...
    """
    def average():
        """!execute average for each image, (preloading 100 image by 100 image to limit memory costs)
    ...
    if allCFD == True:
        combinaison = getAllGenreEthnieCFD(labels_path)
